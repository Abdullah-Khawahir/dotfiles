...
local function set_keymap(run_command, compiler, opt)
  ...
  vim.api.nvim_buf_set_keymap(0, 'n', '<leader>m', lua_run_command, { desc = 'Run [M]ake Program', noremap = true, silent = true })

  vim.cmd('compiler ' .. compiler)
  vim.api.nvim_buf_set_keymap(0, 'n', '<leader>wb', ':make' .. (opt.compiler_opt or '') .. '<CR>', { desc = 'Run WorkSpace [B]uild' })
end
...
vim.api.nvim_create_autocmd('FileType', {
  ...
  callback = function()
    ...
    vim.api.nvim_create_user_command('RazorPage', function(opts)
      ...
      local cmd = string.format('dotnet new page -n %s -o %s --namespace %s.Pages', name, dir, project_name)
...
vim.api.nvim_create_autocmd('fileType', { -- python
  pattern = { 'python' },
  callback = function()
    local run_command = './.venv/bin/python %'
    local compiler = 'pylint'
    set_keymap(run_command, compiler)
    vim.api.nvim_buf_set_keymap(0, 'n', '<leader>i', ':LspPyrightOrganizeImports<CR>', { desc = 'Organize Imports' })
